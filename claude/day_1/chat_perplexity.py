"""
Perplexity - two ways to make the same call.

1. OpenAI-compatible endpoint - via the generic `openai` SDK.
2. Native REST API            - Perplexity does not publish its own
                                 Python SDK; its documented integration
                                 path is raw HTTP against its REST API,
                                 shown here via `requests` instead of
                                 depending on the `openai` package.
...
def call_via_openai_compatible() -> str:
    ...
    # Perplexity's "sonar" models can browse the web as part of the same call.
